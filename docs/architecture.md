...
`backend/app/services/experimental_linkedin_capture/` is a disabled-by-default boundary for possible future user-supervised local LinkedIn capture. Phase 17A added schemas, URL/currentJobId identity helpers, duplicate-reference utilities, diagnostic event codes, a future adapter protocol, and a dry-run runner. Phase 17B adds a mock adapter that emits fake demo jobs, duplicate/currentJobId diagnostics, page/job limit diagnostics, and a local ignored run package under `backend/data/experimental_capture/`. Phase 17C adds a Windows/local selected-job-only prototype that can copy the focused browser URL and visible page text for one job the user already selected manually. Phase 17D adds a focus handoff countdown so the user can switch from JOLT back to the LinkedIn tab before keyboard/clipboard capture begins.

The feature flag is `JOLT_ENABLE_EXPERIMENTAL_LINKEDIN_CAPTURE=false` by default. When disabled, `/api/experimental-capture/linkedin/health`, `/start`, `/stop`, and `/status` return clear disabled/no-op responses. When enabled, `/start` supports `mode=mock_dry_run` and `mode=selected_job_only` with `selected_job_only=true`. `/review-latest` can convert the latest experimental package into the existing Capture -> Parser -> Profile -> Decision review cards. It does not auto-save to History or auto-export.

The selected-job prototype performs no card iteration, no result-panel scrolling, no pagination, no login automation, no credentials, no CAPTCHA/rate-limit bypass, no auto-apply, and no messaging. It does not use Selenium or Playwright. Optional local keyboard/clipboard support is isolated in `windows_selected_job_adapter.py` and returns clear dependency errors when unavailable. Optional dependencies live in `backend/requirements-experimental.txt` and are installed separately with `pip install -r requirements-experimental.txt`.
