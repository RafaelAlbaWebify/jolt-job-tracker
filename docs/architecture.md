...
The selected-job prototype performs no card iteration, no result-panel scrolling, no pagination, no login automation, no credentials, no CAPTCHA/rate-limit bypass, no auto-apply, and no messaging. It does not use Selenium or Playwright. Optional local keyboard/clipboard support is isolated in `windows_selected_job_adapter.py` and returns clear dependency errors when unavailable.
