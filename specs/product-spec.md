...
Phase 17A adds only an experimental LinkedIn capture scaffold: disabled-by-default feature flag, health/start/stop/status API boundaries, raw run package schemas, currentJobId URL utilities, duplicate-reference helpers, and diagnostic status codes. Phase 17B extends that scaffold with fake-data mock dry-run packages and explicit conversion into normal review cards. Phase 17C adds selected-job-only capture for one job the user manually selected, using current URL plus visible copied text. Phase 17D adds a configurable focus handoff countdown before keyboard/clipboard capture starts. Phase 18 ports the previously working legacy local batch workflow into experimental mode with user-supervised card clicking, URL/currentJobId capture, visible detail text capture, duplicate diagnostics, left-panel scrolling, optional start-offset pagination, raw local packages, and review conversion. It must not use Selenium/Playwright, log in, store credentials, bypass protections, auto-apply, send messages, or auto-save experimental output into History.
